...
class AdaBelief_GC(Optimizer):
    r"""Implements AdaBelief_MC algorithm. Modified from AdaBeleif in PyTorch
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(AdaBelief_GC, self).__init__(params, defaults)

        self.weight_decouple = weight_decouple
        self.rectify = rectify
        self.fixed_decay = fixed_decay
        if self.weight_decouple:
            print('Weight decoupling enabled in AdaBelief_GC')
            if self.fixed_decay:
                print('Weight decay fixed')
        if self.rectify:
            print('Rectification enabled in AdaBelief_GC')
        if amsgrad:
            print('AMS enabled in AdaBelief_GC')

    def __setstate__(self, state):
        super(AdaBelief_GC, self).__setstate__(state)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                grad = p.grad.data
                if grad.is_sparse:
                    raise RuntimeError('AdaBelief_GC does not support sparse gradients, please consider SparseAdam instead')
