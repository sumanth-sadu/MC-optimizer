...
class AdaBelief_MC(Optimizer):
    r"""Implements AdaBelief_MC algorithm. Modified from AdaBelief in PyTorch
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(AdaBelief_MC, self).__init__(params, defaults)

        self.weight_decouple = weight_decouple
        self.rectify = rectify
        self.fixed_decay = fixed_decay
        if self.weight_decouple:
            print('Weight decoupling enabled in AdaBelief_MC')
            if self.fixed_decay:
                print('Weight decay fixed')
        if self.rectify:
            print('Rectification enabled in AdaBelief_MC')
        if amsgrad:
            print('AMS enabled in AdaBelief_MC')

    def __setstate__(self, state):
        super(AdaBelief_MC, self).__setstate__(state)
        for group in self.param_groups:
            group.setdefault('amsgrad', False)

    def reset(self):
        for group in self.param_groups:
          # keys present in group : ['params', 'lr', 'betas', 'eps', 'weight_decay', 'amsgrad']
            for p in group['params']:
                state = self.state[p] #dictionary
                amsgrad = group['amsgrad'] #true or false
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                grad = p.grad.data
                if grad.is_sparse:
                    raise RuntimeError('AdaBelief_MC does not support sparse gradients, please consider SparseAdam instead')
                ...
                # modification 15 : replace mt with mt-mean(mt)
